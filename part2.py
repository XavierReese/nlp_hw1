...
class RNNModel(nn.Module):
    def __init__(self, vocab, dims):
        ...
        self.E = torch.nn.Embedding(len(vocab), dims)
        self.W = torch.nn.Linear(dims, dims)
        self.U = torch.nn.Linear(dims, dims)
        self.W_out = torch.nn.Linear(dims, len(vocab))
    ...
    def step(self, h, idx):
        ...
        h_new = torch.tanh(self.W(x) + self.U(h))
    ...
    def predict(self, h, idx):
        ...
        return h_new, self.vocab.denumberize(pred_idx)
    ...
    def fit(self, data, lr=0.001, epochs=10):
        ...
        #	5. Compute the average loss per character by dividing `total_loss / total_chars`.
            ...
            print(f"Epoch {n}: avg_loss={avg_loss:.4f}")
